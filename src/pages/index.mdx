---
metaTitle: Home Page
metaDescription: Home page specific meta description. Cras justo odio, dapibus ac facilisis in, egestas eget quam.
metaImage:
pageTitle: Home
pageIntro: |
  Vivamus **sagittis lacus vel augue** laoreet rutrum faucibus dolor auctor. [In-page link to anchor below](#testTarget). Smooth scrolling is implemented with css `scroll-behavior: smooth;`. Integer posuere erat a ante venenatis dapibus posuere velit aliquet. Duis mollis, est non commodo luctus, nisi erat porttitor ligula, _eget lacinia odio sem nec elit_. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor. Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor.
hasBanner: true
banner:
  bgImage: home.jpg

sections:
  - type: BlockMediaSection
    template: mediaSectionTemplate
    hasBackground: false
    inContainer: true
    sectionID: mediaBlock1
    component: SectionMedia
    targetID: elitPorta
    title: Elit Porta Mollis Cursus Parturient
    subtitle: Nullam id dolor id nibh ultricies vehicula ut id elit.
    content: |-
      Curabitur blandit [tempus porttitor](http://google.com). Donec id elit non mi porta gravida at eget metus. Cras mattis consectetur purus sit amet fermentum. Integer posuere erat a ante venenatis dapibus posuere _velit aliquet_. Donec id elit non mi porta gravida at eget metus. Aenean lacinia bibendum nulla sed consectetur. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus.
    isExternal: false
    linkURL: /page3
    linkText: Try it out
    imageLeft: false
    image: sample5.jpg

  - type: BlockMediaSection
    template: mediaSectionTemplate
    hasBackground: false
    inContainer: true
    sectionID: mediaBlock2
    component: SectionMedia
    targetID:
    title: Elit Dolor Sit
    subtitle: Etiam porta sem malesuada magna mollis euismod. Morbi leo risus, porta ac consectetur ac, vestibulum at eros.
    content: |-
      Nullam id dolor id nibh ultricies vehicula ut id elit. Etiam porta sem malesuada magna mollis euismod. [Scroll to section above](#elitPorta) Maecenas sed diam eget risus varius blandit sit amet non magna. Cras justo odio, dapibus ac facilisis in, egestas eget quam.
    isExternal: false
    linkURL: /page2
    linkText: Check it out
    imageLeft: true
    image: sample3.jpg

  - type: BlockMediaSection
    template: mediaSectionTemplate
    hasBackground: false
    inContainer: true
    sectionID: mediaBlock3
    component: SectionMedia
    targetID:
    title: Consectetur Ultricies Curabitur blandit tempus porttitor
    subtitle: Vestibulum Dolor Cras Elit
    content:
    isExternal: true
    linkURL: https://www.apple.com
    linkText: Go External
    imageLeft: false
    image: sample11.jpg

  - type: Block1
    template: blocktemplate
    hasBackground: true
    inContainer: true
    title: I am component1 of the home page.
    sectionID: home_page_block1
    component: Section1
    content: |-
      Curabitur blandit [tempus porttitor](http://google.com). Donec id elit non mi porta gravida at eget metus. Cras mattis consectetur purus sit amet fermentum. Integer posuere erat a ante venenatis dapibus posuere _velit aliquet_. Donec id elit non mi porta gravida at eget metus. Aenean lacinia bibendum nulla sed consectetur. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus.

      Maecenas faucibus mollis interdum. **Vivamus sagittis lacus vel augue** laoreet rutrum faucibus dolor auctor. Nulla vitae elit libero, a pharetra augue. Cras mattis consectetur purus sit amet fermentum. Morbi leo risus, porta ac consectetur ac, vestibulum at eros.

  - type: Block2
    template: blocktemplate
    hasBackground: true
    inContainer: false
    title: I am component2 of the home page.
    sectionID: home_page_block2
    component: Section2
    content: |-
      1. List item 1
      2. List item 2
      3. List item 3
      4. List item 4
      5. List item 5

  - type: Block3
    template: blocktemplate
    hasBackground: false
    inContainer: true
    title: I am component3 of the home page.
    sectionID: home_page_block3
    component: Section3
    content: >-
      Curabitur blandit tempus porttitor. Donec id elit non mi porta gravida at eget metus. Cras mattis consectetur purus sit amet fermentum. [Integer posuere erat](http://apple.com) a ante venenatis dapibus posuere _velit aliquet_. Donec id elit non mi porta gravida at eget metus. Aenean lacinia bibendum nulla sed consectetur. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus.
      Maecenas faucibus mollis interdum. Laoreet rutrum faucibus dolor auctor. Nulla vitae elit libero, a pharetra augue. Cras mattis consectetur purus sit amet fermentum. Morbi leo risus, porta ac consectetur ac, vestibulum at eros **Vivamus sagittis lacus vel augue**.

      <div>How is this</div>

      Cras justo odio, dapibus ac facilisis in, egestas eget quam. Cras mattis consectetur purus sit amet fermentum. Nullam quis risus eget urna mollis ornare vel eu leo. Morbi leo risus, porta ac consectetur ac, vestibulum at eros.

      Aenean lacinia bibendum nulla sed consectetur. Maecenas faucibus mollis interdum. Curabitur blandit tempus porttitor. Nullam id dolor id nibh ultricies vehicula ut id elit. Maecenas sed diam eget risus varius blandit sit amet non magna. Curabitur blandit tempus porttitor.

      Fusce dapibus, tellus ac cursus commodo, tortor mauris condimentum nibh, ut fermentum massa justo sit amet risus. Maecenas sed diam eget risus varius blandit sit amet non magna. Fusce dapibus, tellus ac cursus commodo, tortor mauris condimentum nibh, ut fermentum massa justo sit amet risus. Nullam quis risus eget urna mollis ornare vel eu leo. Integer posuere erat a ante venenatis dapibus posuere velit aliquet.
      <div id="testTarget">
      Fusce dapibus, tellus ac cursus commodo, tortor mauris condimentum nibh, ut fermentum massa justo sit amet risus. Nulla vitae elit libero, a pharetra augue. Curabitur blandit tempus porttitor. Cras justo odio, dapibus ac facilisis in, egestas eget quam. Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor. Nullam quis risus eget urna mollis ornare vel eu leo.
---

import StandardPage from "../components/layout/standard-page";
export default StandardPage;

## This is MDX content

Duis mollis, est non commodo luctus, nisi erat porttitor ligula, eget lacinia odio sem nec elit. Integer posuere erat a ante venenatis dapibus posuere velit aliquet. Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor. Maecenas faucibus mollis interdum. Duis mollis, est non commodo luctus, nisi erat porttitor ligula, eget lacinia odio sem nec elit. Aenean eu leo quam. Pellentesque ornare sem lacinia quam venenatis vestibulum. Lorem ipsum dolor sit amet, consectetur adipiscing elit.

<InlineMessage text={`Look Ma, a shortcode`} />

Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Integer posuere erat a ante venenatis dapibus posuere velit aliquet. Maecenas sed diam eget risus varius blandit sit amet non magna. Integer posuere erat a ante venenatis dapibus posuere velit aliquet. Curabitur blandit tempus porttitor.
